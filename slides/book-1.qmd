...
1.1. Choose a metric to optimize. For example, you might want to optimize for
impressions—the number of times an ad is shown.
2.2. Collect data and obtain labels.
3.3. Engineer features.
4.4. Train models.
5.5. During error analysis, you realize that errors are caused by the wrong labels, so
you relabel the data.
6.6. Train the model again.
7.7. During error analysis, you realize that your model always predicts that an ad
...
8.8. Train the model again.
9.9. The model performs well on your existing test data, which is by now two months
...
10.10. Train the model again.

11.11. Deploy the model.

12.12. The model seems to be performing well, but then the businesspeople come
...
If you store it in a text file, it’ll require 7 characters, and if each character is
...
1.1. Put the first k elements into the reservoir.
2.2. For each incoming nth element, generate a random number i such that 1 ≤ i ≤ n.
3.3. If 1 ≤ i ≤ k: replace the ith element in the reservoir with the nth element. Else, do
nothing.
86 | Chapter 4: Training Data

This means that each incoming nth element has kn probability of being in the reser‐

voir. You can also prove that each element in the reservoir has kn probability of
...
Px
Qx. Q(x) is called the proposal distribution or the importance distribution. Q(x) can
...
by active learning gives an accuracy of 90%. Source: Burr Settles^22
Labeling | 101
...
resume screening (98% of job seekers are eliminated at the initial resume screening^30 ).
...
Adapted from an image by Rafael Alencar^37
...
that if there are two instances, x 1 and x 2 , and the loss resulting from making the
wrong prediction on x 1 is higher than x 2 , the model will prioritize making the correct
prediction on x 1 over making the correct prediction on x 2. By giving the training
...
as the other way around, you can make C 10 twice as high as C 01.
...
Wi=number of samples of class iN
...
as 1). From example x 1 of label DOG and example x 2 of label CAT, you can generate x'
such as:
x′=γx 1 +1 −γx 2
The label of x' is a combination of the labels of x 1 and x 2 : γ× 0 + 1 −γ × 1. This
...
corresponds to 2^18 = 262,144 possible hashed values, all the categories, even the ones
...
1.1. You start by training the first weak classifier on the original dataset.
2.2. Samples are reweighted based on how well the first classifier classifies them, e.g.,
misclassified samples are given higher weight.
3.3. Train the second classifier on this reweighted dataset. Your ensemble now con‐
sists of the first and the second classifiers.
4.4. Samples are weighted based on how well the ensemble classifies them.
5.5. Train the third classifier on this reweighted dataset. Add the third classifier to the
ensemble.
6.6. Repeat for as many iterations as needed.
7.7. Form the final strong classifier as a weighted combination of the existing classifi‐
...
Adapted from an image by Jim Dowling^17
...
number of hidden layers, number of hidden units, dropout probability, β 1 and β 2 in
...
Source: Adapted from an image by Stack Overflow^14
...
convolution uses only K^2 + C instead of K^2 C parameters. If K = 3, this means an eight
...
neural network. Source: Adapted from an image by TensorRT team^48
...
1.1. It first breaks your computation graph into subgraphs.
2.2. It predicts how big each subgraph is.
3.3. It allocates time to search for the best possible path for each subgraph.
4.4. It stitches the best possible way to run each subgraph together to execute the
entire graph.
...
Source: Adapted from an image by MadeWithML
...
1.1. A model to detect fraudulent transactions
2.2. A model to recommend relevant products to users
3.3. A model to predict whether a seller is abusing a system
4.4. A model to predict how long it will take to ship an order
...
1.1. Pull data.
2.2. Downsample or upsample this data if necessary.
3.3. Extract features.
4.4. Process and/or annotate labels to create training data.
5.5. Kick off the training process.
6.6. Evaluate the newly trained model.
7.7. Deploy it.
...
1.1. Deploy the candidate model in parallel with the existing model.
2.2. For each incoming request, route it to both models to make predictions, but only
serve the existing model’s prediction to the user.
3.3. Log the predictions from the new model for analysis purposes.
...
1.1. Deploy the candidate model alongside the existing model.
2.2. A percentage of traffic is routed to the new model for predictions; the rest
...
3.3. Monitor and analyze the predictions and user feedback, if any, from both models
...
1.1. Deploy the candidate model alongside the existing model. The candidate model
is called the canary.
2.2. A portion of the traffic is routed to the candidate model.
3.3. If its performance is satisfactory, increase the traffic to the candidate model. If
not, abort the canary and route all the traffic back to the existing model.
4.4. Stop when either the canary serves all the traffic (the candidate model has
...
1.1. Download the latest PyTorch base image.
2.2. Clone NVIDIA’s apex repository on GitHub, navigate to the newly created apex
folder, and install apex.
3.3. Set fancy-nlp-project to be the working directory.
4.4. Clone Hugging Face’s transformers repository on GitHub, navigate to the newly
...
1.1. Pull last week’s data from data warehouses.
2.2. Extract features from this pulled data.
3.3. Train two models, A and B, on the extracted features.
4.4. Compare A and B on the test set.
5.5. Deploy A if A is better; otherwise deploy B.
...
K8s can be used on-prem (even on your laptop via minikube). However, I’ve
